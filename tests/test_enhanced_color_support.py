...
from src.ir.color import IrColor, parse_color, _get_svg_color
  ...
  def test_basic_svg_colors(self):
    ...
    for name, expected_hex in basic_colors.items():
      ...
      if name == "transparent":
        ...
        assert actual_hex == expected_hex, f"Color {name}: expected {expected_hex}, got {actual_hex}"

  def test_extended_svg_colors(self):
    """Test extended SVG color keywords."""
    extended_colors = [
      "cornflowerblue", "mediumseagreen", "darkslategray", 
      "lightgoldenrodyellow", "palevioletred", "springgreen"
    ]
