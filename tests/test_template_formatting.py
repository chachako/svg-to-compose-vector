...
<svg width="100" height="100" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
  ...
  def test_composable_function_template_indentation(self, simple_svg):
    ...
    assert not composable_line.startswith(' '), f"@Composable should not be indented: {composable_line}"
    ...
    nested_param_lines = [line for line in lines if ('name =' in line or 'defaultWidth =' in line) and line.startswith('      ')]
  ...
  def test_icon_object_template_indentation(self, simple_svg):
    ...
    assert not object_line.startswith(' '), f"Object declaration should not be indented: {object_line}"
  ...
  def test_val_declaration_template_indentation(self, simple_svg):
    ...
    # Check parameter lines (2 spaces indentation)
    param_lines = [line for line in lines if ('name =' in line or 'defaultWidth =' in line) and not line.strip().startswith('val')]
    for line in param_lines:
      assert line.startswith('  '), f"Parameter line should have 2 spaces: '{line}'"
...
ImageVector.Builder(
  ...
  path(
    ...
    assert result_normalized == expected_normalized, f"Output mismatch:\n\nActual:\n{result_normalized}\n\nExpected:\n{expected_normalized}"
...
fun HomeIconIcon(
  ...
  def test_complex_svg_formatting_with_groups(self, complex_svg_with_groups):
    ...
    nested_content_lines = [line for line in lines if line.startswith('    ') and ('path(' in line or 'group(' in line)]
    ...
    assert any('import androidx.compose.ui.graphics.Brush' in line for line in lines), "Should include Brush import"
    assert any('import androidx.compose.ui.geometry.Offset' in line for line in lines), "Should include Offset import"
    ...
    for line in gradient_lines:
      ...
      assert line.strip().startswith('fill = Brush.linearGradient'), f"Gradient should be fill parameter: '{line}'"
  ...
  def test_imports_grouping_affects_output(self, complex_svg_with_groups):
    ...
    assert len(import_section_grouped) >= len(import_section_ungrouped), "Grouped imports should have more lines due to spacing"
  ...
  def test_line_endings_consistency(self, simple_svg):
    ...
    for template_name in templates:
      ...
      for i, line in enumerate(lines):
        assert not line.endswith(' '), f"Template {template_name} line {i+1} should not have trailing spaces: '{line}'"
      ...
      for line in lines:
        if line.strip() == '':
          empty_line_count += 1
          assert empty_line_count <= 2, f"Template {template_name} should not have more than 2 consecutive empty lines"
        else:
          empty_line_count = 0