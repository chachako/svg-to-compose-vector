...
<svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  ...
  <path d="M0 0h24v24H0z" fill="url(#grad)"/>
    ...
    gradient_lines = [
      line for line in lines if "colorStops" in line or "start =" in line or "end =" in line
    ]
    ...
    for template_name in templates:
      ...
      for i, line in enumerate(lines):
        assert not line.endswith(" "), (
          f"Template {template_name} line {i + 1} has trailing space: '{line}'"
        )
        assert not line.endswith("\t"), (
          f"Template {template_name} line {i + 1} has trailing tab: '{line}'"
        )
      ...
      if import_lines:
        ...
        assert lines[last_import_line + 1] == "", (
          f"Template {template_name} should have empty line after imports"
        )
        assert lines[last_import_line + 2] != "", (
          f"Template {template_name} should not have multiple empty lines after imports"
        )
