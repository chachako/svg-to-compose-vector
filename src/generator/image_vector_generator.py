...
class ImageVectorGenerator:
  ...
  def _generate_path(self, path: IrVectorPath) -> List[str]:
    ...
    has_parameters = (
      path.fill is not None or 
      path.stroke is not None or
      path.fill_alpha != 1.0 or
      path.stroke_alpha != 1.0 or
      path.stroke_line_width != 0.0 or
      path.stroke_line_cap != "butt" or
      path.stroke_line_join != "miter" or
      path.stroke_line_miter != 4.0 or
      path.path_fill_type != "nonZero"
    )
  ...
  def _generate_group(self, group: IrVectorGroup) -> List[str]:
    ...
    has_transform = (
      group.rotation != 0.0 or
      group.pivot_x != 0.0 or
      group.pivot_y != 0.0 or
      group.scale_x != 1.0 or
      group.scale_y != 1.0 or
      group.translation_x != 0.0 or
      group.translation_y != 0.0
    )
